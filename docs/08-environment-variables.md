...
| `NEXT_PUBLIC_SENTRY_DSN` | No | `https://...@sentry.io/...` | Sentry DSN. Optional for local dev; enables error tracking in production. |
| `SENTRY_ORG` | No (build-time) | `your-org-slug` | Sentry org slug — server-side, Vercel CI only. |
| `SENTRY_PROJECT` | No (build-time) | `your-project-slug` | Sentry project slug — server-side, Vercel CI only. |
| `SENTRY_AUTH_TOKEN` | No (build-time) | `sntrys_...` | **Secret** — never expose as `NEXT_PUBLIC_*`. Vercel CI only, used for source map uploads. |
