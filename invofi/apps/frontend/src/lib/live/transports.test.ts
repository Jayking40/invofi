...
describe('decodeEnvelope', () => {
  it('decodes position_updated', () => {
    expect(
      decodeEnvelope(
        JSON.stringify({ type: 'position_updated', positionId: 'off_1', fields: { status: 'Financed' } }),
      ),
    ).toEqual({ kind: 'position_updated', positionId: 'off_1', fields: { status: 'Financed' } });
  });
  ...
  it('decodes yield_calculated with string bigint amounts', () => {
    ...
    ).toEqual({ kind: 'yield_calculated', positionId: 'off_1', apy: 12.5, earnedToDate: 250_000n });
  });

  it('decodes repayment_received', () => {
    expect(
      decodeEnvelope(
        JSON.stringify({
...
describe('createPollingTransport', () => {
  ...
  it('maps inv_rep events to repayment updates and requests a resync', () => {
    type TestEvent = {
      type: string;
      subjectId: string;
